...
export default function VideoThumbnail({
  ...
  const startButtonIndex =
    Math.floor((currentPage - 1) / buttonsPerPage) * buttonsPerPage;
  ...
  function closePlayer() {
    ...
    const viewportWidth = hasScrollbar
      ? window.innerWidth
  ...
  useEffect(() => {
    if (platform === "savedlist") {
      if (address) {
              ...
              {visibleResults.map((item, index) => (
                ...
                >
                  ...
                  <Link to={`/${item.platform}/${item.video_id}/`}>
                    <Image
                      src={item.image_url}
                      getFallbackSrc={() => [`/assets/no-image.webp`]}
                      alt={item.video_id}
                      title={item.video_id}
                      width="214"
                      height="142"
        ...
        setVideoThumb(
          <div
            ...
            style={{ color: "var(--red-bg)", fontSize: 24, fontWeight: 600 }}
          >
      ...
      if (visibleResults.length === 0) {
        if (
          isSearch === "search" &&
          (platform === "eporner" || platform === "redtube")
        ) {
          setVideoThumb(
            <>
        ...
        let videoId,
          ...
          getThumbnailUrl,
          viewCount,
          getOnliveStatus = false,
          isHaveViewCount = false,
          isHaveSearchForm = false;
        ...
        if (platform === "camsoda") {
          ...
          getThumbnailUrl = (item) =>
            `https://laving.cc/${item.file_code}_xt.jpg?v=1`;
              ...
              {visibleResults.map((item, index) => {
                ...
                return (
                  <div
                    ...
                    className="thumb"
                  >
                    {getOnliveStatus && !item.live && (
                      <div className="thumb-overlay">OFFLINE</div>
                    )}
                    <Link
                      to={`/${platform}/${item[videoId]}/`}
        ...
        <>
          {isNotFound ? (
            <div className="notfound-page">
              <img
                src="/assets/gif-lesbico-sumisa-gimiendo-anime-porn.gif"
                alt="gif-lesbico-sumisa-gimiendo-anime-porn"
                width="1024"
                height="576"
              />
            </div>
